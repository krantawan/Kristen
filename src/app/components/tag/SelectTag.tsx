...
export default function SelectTag({ selectedTags, setSelectedTags }: Props) {
  ...
  const [visibleGroups, setVisibleGroups] = useState<string[]>(["Class", "Trait Tags", "Tag Tier"]);
      ...
      <div className="flex flex-wrap gap-3 mb-4 items-center justify-between w-full">
        <div className="flex flex-wrap gap-3">
          {allGroupLabels.map(label => (
            <label key={label} className="flex items-center gap-1 text-sm text-[#f3f3f3]">
              <input
                type="checkbox"
      ...
      {visibleGroups.length > 0 && (
        <div className="flex flex-col gap-4 my-2">
          {tagGroups
            ...
            .map(group => (
              <div key={group.label}>
                <p className="text-sm font-bold text-[#BEC93B] mb-1">{group.label}</p>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 font-bold text-sm rounded ${
                        selectedTags.includes(tag)
                          ? tag === "Top Operator" ? "bg-[#FFD802] text-black" : "bg-[#BEC93B] text-black"
